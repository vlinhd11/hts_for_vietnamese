...
PROGNAME=$(basename $0)
source /extr-data/conf/Config.cfg || exit 1
function error_exit
{
...
if [[ ! -z $OUT_DIR ]]; then
    odir=$odir/$OUT_DIR   # This can be set from environment variable
fi
...
function process
{
    ...
    if [[ $analyzer == "world" ]]; then
        ./prog/world.sh $wav $index $odir $subDir  || exit 1 # Given audio and output folder, output mgc, lf0, [bap], and cmp file
    else
        ./prog/sptk.sh $wav $index $odir $subDir  || exit 1 # Given audio and output folder, output mgc, lf0, [bap], and cmp file
    fi
}
...
mkdir -p $odir/scp
find $odir/cmp -iname "*.cmp" | sed "s:${odir}:/data:g"  | sort > $odir/scp/train.scp  # We use /data to make it compatible with the tts-training container
